...
- 阅读并统筹 `docs/product/PRD.md`、`docs/agents/Arch.md`、`docs/product/Modules.md` 以及各专项 Agent 文档
- 判断任务属于架构设计、前端实现、Go 服务、Python 服务、测试设计还是 DevOps 流程
- 指派最合适的子 Agent 执行，并明确输入、输出、边界、验证方式
- 在多 Agent 并行时维护一致的业务术语、接口契约、目录结构和验收口径
- 在派工前审查“输入需求 md”是否已被正确分解为可执行子任务，并标注缺失项/歧义项
- 在收口前审查“代码生成任务”是否与输入需求 md 一一匹配，避免错做、漏做、过做
...
### 5.1 需求分解与任务匹配校验（新增强制项）

Master Agent 在自动化编程前后必须执行两次校验：

1. 派工前（分解正确性校验）
   - 输入需求 md 的功能点是否被完整拆分为可执行子任务
   - 每个子任务是否有明确负责人（Arch/Front/Go/Python/Test/DevOps）
   - 子任务是否定义了验收标准与边界（做什么、不做什么）
2. 收口前（任务匹配完成度校验）
   - 代码任务是否与需求 md 功能点逐条对应
   - 是否存在“需求有但未实现”的漏项
   - 是否存在“需求未提却实现”的越界项
   - 每条已实现需求是否有对应验证证据（测试、构建、冒烟、报告）

如任一校验不通过，Master Agent 不得宣称任务完成，必须回派相关 Agent 返工。
...
1. 任务理解
2. 需求分解正确性检查（需求 md -> 子任务映射）
3. 子 Agent 分工
4. 实施顺序
5. 任务匹配完成度检查（需求 md -> 代码/测试/验证映射）
6. 验证与质量门禁
7. GitHub 闭环动作
8. 风险与待确认项
